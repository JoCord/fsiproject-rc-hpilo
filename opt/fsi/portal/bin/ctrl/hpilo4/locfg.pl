...
## Version 4.10b (modified SSL_verify_mode=>SSL_VERIFY_NONE)
##
...
use constant VERSION       => "4.10";                                                                                              #program version, sent with HTTP headers
...
my ( $socket, $server, $logfile, $infile, $help, $uname, $pword, $ilo2, $ilo3, $verbose, $interactive, $firmware, $firmwarelen, $firmwarebuf, $cookie, $ln, $response, $RIBCLbusy, $retry, $ConnectionErrorMessage, %valuepair, );
$cookie                 = "";
$RIBCLbusy              = 0;
$retry                  = 0;
$verbose                = 0;
...
$result = index( $server, $char );
if ( $result != -1 ) {

   my $start = index( $server, $startBracket );
   my $end   = index( $server, $endBracket );

   if ( $start != -1 && $end != -1 ) {
      my $subname = substr( $server, $end );
      $server = substr( $server, $start + 1, $end - 1 );
      my $colon = ":";
      $result = index( $subname, $colon );
      if ( $result != -1 ) {
         $port = substr( $subname, $result + 1 );
      }
   } elsif ( $start == -1 && $end == -1 ) {
      my $colon = ':';
      $result = index( $server, $colon );
      if ( $result != -1 ) {
         $result = $result + 1;
         $port   = substr( $server, $result );
         $result = $len - length($port) - 1;
         $server = substr( $server, 0, $result );
      } ## end if ( $result != -1 )
   } else {
...
if ( !$ilo2 && !$ilo3 ) {
   ...
   if ( $ipV6 == 1 ) {
      ...
      socket( $socket, PF_INET, SOCK_STREAM, 0 ) || die "Socket Creation";
      connect( $socket, pack_sockaddr_in( $port, inet_aton($server) ) ) or die "connect: $!";
   }
...
if ($ilo2) {
   ...
   while ( $ln = <IN> ) {
      ...
      if ( ( ( $ln =~ /<[ \t]*LOGIN[ \t]/ ) || ( $ln =~ /<[ \t]*LOGIN$/ ) ) && ($pword) && ($uname) ) {
         ...
         next;
      } ## end if ( ( ( $ln =~ /<[ \t]*LOGIN[ \t]/ ) || ( $ln =~ /<[ \t]*LOGIN$/ ) ) && ($pword) && ($uname) )
...
if ($updateribfwcmd) {                                                                                                             # it's a firmware update
   ...
   my (
        $body1, $body1size,                                                                                                          # multipart body
   ...
   } ## end while ( $sentbytes < $firmwarelen )
   printf "\n\n";
...
sub send_or_calculate                                                                                                              # used for iLO 3 and iLO 4 only
{
   ...
   while ( $ln = <IN> ) {
      ...
      if ( $ln =~ m/UPDATE_RIB_FIRMWARE/i ) {
         ...
         # print "\n----- ERROR: syntax error detected in $ln\n" if ($verbose);
      } ## end if ( $ln =~ m/UPDATE_RIB_FIRMWARE/i )
...
sub read_chunked_reply                                                                                                             # used for iLO 3 and iLO 4 only
{
   ...
   while (1) {
      ...
      if ($hide) {
         ...
         } ## end else [ if ( $chunkSize == length($ln) ) ]
...
sub do_variable_substitutions {
   if ( keys(%valuepair) > 0 ) {
      for my $key ( keys %valuepair ) {
         ...
         # print "process_variable_substitution $key => $value\n";
         $_[ 0 ] =~ s/\%$key\%/$value/g;
      } ## end for my $key ( keys %valuepair )
   } ## end if ( keys(%valuepair) > 0 )

   # print "----- $_[0]\n" if ($verbose);
} ## end sub do_variable_substitutions
